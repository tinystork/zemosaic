...
def launch_filter_interface(
    ...
    **kwargs,
    ...
    try:
        ...
        compute_auto_caps_func = None
        if policy_mod:
            compute_auto_cap_func = getattr(policy_mod, 'compute_auto_max_raw_per_master_tile', None)
            compute_auto_caps_func = getattr(policy_mod, 'compute_auto_tile_caps', None)
        ...
        if compute_auto_caps_func is None and worker_mod:
            compute_auto_caps_func = getattr(worker_mod, '_compute_auto_tile_caps', None)
        ...
        def _auto_organize_master_tiles() -> None:
            ...
            try:
                ...
                caps_info_for_merge: Dict[str, Any] = {}
                if callable(compute_auto_caps_func):
                    try:
                        caps_info_for_merge = compute_auto_caps_func(
                            resource_info,
                            per_frame_info,
                            policy_max=50,
                            policy_min=8,
                            user_max_override=manual_master_cap_value if manual_master_cap_value > 0 else None,
                        ) or {}
                    except Exception as exc:
                        _log_message(f"Auto-cap policy probe failed: {exc}", level="DEBUG_DETAIL")
                        caps_info_for_merge = {}
                ...
                if caps_info_for_merge and isinstance(cap_details, dict):
                    try:
                        cap_details.setdefault("min_cap", int(caps_info_for_merge.get("min_cap", 0)))
                    except Exception:
                        pass
                ...
                merge_floor = 0
                try:
                    merge_floor = int(cap_details.get("min_cap", 0) if isinstance(cap_details, dict) else 0)
                except Exception:
                    merge_floor = 0
                if merge_floor > 0 and cap_value_int > 0 and final_groups:
                    merge_floor = max(1, min(merge_floor, cap_value_int))
                    merged_flags = [False] * len(final_groups)
                    centers_cache = [_group_center_deg(grp) for grp in final_groups]
                    for idx, grp in enumerate(final_groups):
                        if merged_flags[idx] or len(grp) >= merge_floor:
                            continue
                        best_j: Optional[int] = None
                        best_d = float("inf")
                        for jdx, _ in enumerate(final_groups):
                            if jdx == idx or merged_flags[jdx]:
                                continue
                            dist = _angular_sep_deg(centers_cache[idx], centers_cache[jdx])
                            if dist < best_d:
                                best_d = dist
                                best_j = jdx
                        if best_j is not None and len(final_groups[best_j]) + len(grp) <= int(cap_value_int * 1.1):
                            final_groups[best_j].extend(grp)
                            merged_flags[idx] = True
                            _log_message(
                                (
                                    f"[FilterAuto] Merge group {idx} ({len(grp)} imgs) into {best_j} "
                                    f"(now {len(final_groups[best_j])})"
                                ),
                                level="INFO_DETAIL",
                            )
                    if any(merged_flags):
                        final_groups = [grp for idx, grp in enumerate(final_groups) if not merged_flags[idx]]
