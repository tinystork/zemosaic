...
## 1) zemosaic_worker.py — Phase 5 tile preparation (assemble_final_mosaic_reproject_coadd) [x]
...
## 2) zemosaic_worker.py — Phase 5 per-channel coadd call: pass input_weights [x]
...
## 3) GPU forcing (correctness-first) [x]
...
## 4) zemosaic_utils.py — ensure wrapper forwards input_weights to CPU [x]
