    def _best_effort_anchor_photometry() -> None:
        ...
        if len(effective_tiles) < 2:
            return
        if final_output_wcs is None:
            logger.info("existing_master_tiles_mode: best-effort anchor skipped (missing final_output_wcs)")
            return
        ...
        for idx_entry, entry in enumerate(effective_tiles):
            ...
            if not np.all(np.isfinite(gains)):
                logger.info("existing_master_tiles_mode: skip photometric anchor (non-finite gain)")
                continue
            try:
            ...
            if applied_channels > 0:
                try:
                    ...
                    _pcb(
                        "existing_master_tiles_anchor_applied",
                        prog=None,
                        lvl="INFO_DETAIL",
                        tile_idx=idx_entry,
    ...
    try:
        ...
        logger.warning("existing_master_tiles_mode: best-effort anchor aborted: %s", anchor_exc, exc_info=True)
