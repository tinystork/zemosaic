...
def gpu_reproject_and_coadd_impl(data_list, wcs_list, shape_out, **kwargs):
    ...
    # Keep ``match_background`` enabled even when affine corrections are provided so
    # that the GPU pipeline mirrors the CPU workflow. The CPU path applies affine
    # gain/offset first and still relies on ``match_background`` to remove residual
    # sky offsets, which is essential to avoid seams when tiles only partially
    # overlap or when the affine solve is mildly under-constrained.
